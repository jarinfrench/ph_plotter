...
def run(variables):
    ...
    if plot_style == "mesh":
        from ph_plotter.band_sf_mesh_plotter import BandSFMeshPlotter as BandSFPlotter
...
def main():
    ...
    parser.add_argument("--ninterp", type=int, help="Interpolation number.")
    parser.add_argument(
        "--selected_irreps",
        type=json.loads,
    ...
    parser.add_argument(
        "--combinations_elements",
        nargs="+",
        type=str,
        help="Specification of Combinations of elements. ex. Cu Cu Au Au",
    )
    parser.add_argument(
        "--elements", nargs="+", type=str, help="Specification of elements. ex. Cu Au"
    )
