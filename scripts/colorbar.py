...
class Colorbar(object):
    ...
    def plot(self, figsize, figure_type, save):
        cmap = self._cmap
        bounds = self._bounds
        fig, axarr = plt.subplots(1, 1, figsize=figsize, tight_layout=True)
...
def main():
    ...
    parser.add_argument(
        "--range", nargs=2, default=[-1.0, 1.0], type=float, help="Plotting range"
    )
    parser.add_argument("--pitch", default=1.0, type=float, help="Plotting pitch")
    parser.add_argument(
        "-a",
        "--alpha",
        default=0.2,
    ...
    parser.add_argument(
        "-t", "--figure_type", default="pdf", type=str, help="Filetype of figures."
    )
    parser.add_argument(
        "--figsize",
        nargs=2,
    ...
    parser.add_argument(
        "-s", "--save", action="store_true", help="Save the colorbar figure"
    )
